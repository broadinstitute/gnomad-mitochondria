...
):  # disease-associated variant list, and MITOMAP database allele frequencies
    mitomap_counts = {}
    matches_disease = {}
...
def hmtvar(
    insilicos_path,
):  # in silico prediction for tRNA variants, retrieved via api using get_hmtvar.py
                ...
                if vep.count("|") > header.count(
                    ...
                    if (BIOTYPE == "Mt_tRNA") and (VARIANT_CLASS == "SNV"):
                        if not (SYMBOL, POS, ALT) in matches_insilico:
                            ...
                            pon_mt = info.split(";pon_mt_trna_prediction=")[1].split(
                                ";"
                            )[0]
                            matches_insilico[(SYMBOL, POS, ALT)] = (mitotip, pon_mt)
            ...
            if not row[0].startswith("#"):
                if row[6] == "PASS":  # pass only sites
                    ...
                    file.write(
                        ...
                        + str(ALT)
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][0])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][1])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][2])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][3])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][4])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][5])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][6])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][7])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][8])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][9])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][10])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][11])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][12])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][13])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][14])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][15])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][16])
                        + "\t"
                        + str(matches_annotations[POS, REF, ALT, SYMBOL][17])
                        + "\t"
                        + str(max_base)
                        + "\t"
                        + str(max_codon)
...
def consequences(
    synthetic_vcf_path,
):  # create dictionaries of the consequences and locus of each possible SNV in mtDNA
    with open(
...
def in_helix(
    other_databases_path,
):  # allele frequency and maximum heteroplasmy data from HelixMTdb database
