...
##############################################################
cp $SOURCE_DIR/zcombined_unfiltered.vcf.bgz zcombined_unfiltered.vcf.gz 
...
# VEP annotations
cp /humgen/atgu1/fs03/kristen/mitochondri22_2020/t21/vep_unfiltered.vcf.bgz vep_unfiltered.vcf.gz 
gunzip vep_unfiltered.vcf.gz 

############################################
# convert from gvcf into sample2var format for non-zero values
############################################
# this excludes variants that were never pass

./gvcf_to_table.nosplit.pl -pass 6 -m 0.0001 -dpcol 0 -vafcol 1 -filtercol 4 zcombined_unfiltered.vcf > all.sample2var.unfiltered.noannot.txt

# this version includes all variants (including artifact prone and npg=no pass genotype)
./gvcf_to_table.nosplit.pl -m 0.01 -dpcol 0 -vafcol 1 -filtercol 4 zcombined_unfiltered.vcf > all.sample2var.unfiltered.noPassFilter.m01.txt

############################################
# Add 2 annotations to unfiltered_sample_annotations.annot.txt (file has 1 line per sample):
#   release.3.1.1: "true" if sample included in final release
...
#
# to do this we merge in the colunmns we want from other files
# note the first column in SAMPLE_ID (with spaces) but need to merge via participant_id (no spaces)
#
############################################
...
cat all.sample2var.unfiltered.txt | grep -v -P "(FAIL|base_qual|position|strand_bias|weak_evidence|black)" > all.sample2var.passplus.txt
